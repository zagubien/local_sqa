...
DEFAULT_MODEL_DIR = "/net/vol/zigor/checkpoints/27"
...
def collect_system_files(items, system_id: str):
    out = []
    for fn, mos in items:
        if not fn.startswith(f"{system_id}-"):
            continue
        p = BVCC_WAV_ROOT / fn
        if not p.exists():
            continue
        dur = wav_duration_s(p)
        out.append((fn, float(mos), p, float(dur)))
    if not out:
        raise ValueError(f"no files found for {system_id}")
    return out


def build_mixed_packet(sys_a, sys_b, sysA_id: str, sysB_id: str, block: int,
    ...
    while total_s < target_seconds and len(packet) < max_files:
        for _ in range(block):
            ...
            fn, mos, p, dur = a[ia]
            ia += 1
            packet.append((sysA_id, fn, float(mos), str(p), float(dur)))
            total_s += float(dur)
        ...
        for _ in range(block):
            ...
            fn, mos, p, dur = b[ib]
            ib += 1
            packet.append((sysB_id, fn, float(mos), str(p), float(dur)))
            total_s += float(dur)
...
def save_packet_csv(path: Path, sysA: str, sysB: str, schedule_name: str, block: int, packet,
                    ...
                    target_seconds: float, start_index: int, allow_repeat: bool):
...
def run_pair_schedule(predictor_gpu, predictor_cpu, run_root: Path,
    ...
    if not pkt_path.exists():
        packet_raw, total_s, nA, nB = build_mixed_packet(
            sysA_list, sysB_list, sysA_id, sysB_id, block,
        ...
        save_packet_csv(
            pkt_path, sysA_id, sysB_id, schedule_name, block,
    ...
    for pause_dirname, pause_kind in PAUSE_MODES:
        ...
        with open(p_rm, "w", newline="") as f:
            ...
            for i, (src, fn, mos_t, path, dur) in enumerate(packet):
                ...
                ok, rt, mem, mos_pred, where = safe_infer_global(
                    ...
                    f"rm:{schedule_name}:{pair_name}:{pause_dirname}:{i}",
        ...
        with open(p_concat, "w", newline="") as f:
            ...
            for i, (src, fn, mos_t, path, dur) in enumerate(packet):
                ...
                if i < len(packet) - 1:
                    seed = zlib.adler32(f"{schedule_name}|{pair_name}|{pause_kind}|{i}".encode("utf-8")) & 0xFFFFFFFF
                ...
                ok, rt, mem, mos_pred, where = safe_infer_global(
                    ...
                    f"cat:{schedule_name}:{pair_name}:{pause_dirname}:{i}",
                    fallback_to_cpu=fallback_to_cpu,
                )
                if not ok:
...
def main():
    ...
    ap.add_argument("--pause_seconds", type=float, default=1.0)
    ...
    run_root = Path(model_dir) / "results" / "two_speaker"
    ...
    for sid in SYSTEM_IDS:
        ...
        sys_map[sid] = collect_system_files(items, sid)
        print(f"  files: {len(sys_map[sid])}")
    ...
    pairs = list(combinations(SYSTEM_IDS, 2))
    print(f"\nall pairs: {len(pairs)}")

    for sysA, sysB in pairs:
        for schedule_name, block in SCHEDULES:
            try:
                ...
                print(f"\n[error] {sysA}_{sysB} / {schedule_name}: {e}\n")
