...
# dialogue speakers (bvcc systems)
SYSTEM_IDS = ["sys6c11c", "sys8f532", "sys78aec", "sys83aed", "sys91caa", "sysd81da"]

MODES_TO_RUN = ["forward", "reverse", "random"]

TARGET_SECONDS = 180.0
BLOCK_SECONDS = 20.0  # <-- requested 1s
START_INDEX = 0
...
# keep variants like your other scripts
VARIANTS = ["original", "no_pause", "long_pause", "long_pause_noise"]

# pause configs (used for variants, same semantics as before)
...
def parse_bvcc_list(list_path: Path) -> List[Tuple[str, float]]:
...
_SYS_RE = re.compile(r"(sys[0-9a-f]{5})", re.IGNORECASE)


def extract_sys(utt: str) -> str:
    m = _SYS_RE.search(utt)
    if m:
...
def build_dialogue_packet(items: List[Tuple[str, float]], sys_a: str, sys_b: str) -> List[Tuple[str, float, Path, float]]:
    ...
    while total_s < TARGET_SECONDS and (ia < len(a_list) or ib < len(b_list)):
        for pick_a in (True, False):  # A then B
            ...
            out.append((utt, float(mos), p, dur))
            total_s += dur
...
def compute_blocks(
    ...
    # block_rm: dur-weighted mean of raw segment preds inside each block (split at boundaries)
    # block_concat: concat audio pieces inside block and infer once
...
def build_run_dir(model_dir: Path) -> Path:
    ...
    run_dir = run_root / f"bvcc_dialogue_{ckpt_tag}_t{int(TARGET_SECONDS)}_s{START_INDEX}_block{int(BLOCK_SECONDS)}"
...
def run_training(train_id: int) -> None:
    ...
    for sys_a, sys_b in pairs:
        ...
        meta = {
            ...
            "list_path": str(BVCC_TEST_LIST),
            "wav_root": str(BVCC_WAV_ROOT),
        }

        for mode in MODES_TO_RUN:
            base_out = out_dir_for(run_dir, mode, sys_a, sys_b)

            # skip if done (all variants have all 4 csv)
            if SKIP_IF_DONE:
                done = True
                for v in VARIANTS:
                    vd = base_out / v
            ...
            for utt, mos_t, p, dur in packet:
                wav = load_wav(p)
                seed = _make_seed("bvcc_dialogue", str(train_id), sys_a, sys_b, utt)
                variants = make_variants(wav, SAMPLING_RATE, seed)
                segs.append(Seg(utt=utt, mos_t=float(mos_t), path=p, dur_s=float(dur), wav=wav, variants=variants))
            ...
            for variant in VARIANTS:
                ...
                # -------- segment preds (raw) for this variant
                seg_pred_raw: List[float] = []
