...
DIALOGUE_PREFIX = "SOMOS_dialogue_"
...
def iter_pause_dirs(dialogue_root: Path) -> List[Path]:
    ...
    for sch in SCHEDULE_DIRS:
        ...
        for p in base.rglob("pause_*"):
            if p.is_dir():
                pause_dirs.append(p)
    return sorted(set(pause_dirs), key=lambda x: str(x))
...
def main() -> None:
    ...
    for _tid, root in dialogue_roots:
        pause_dirs = iter_pause_dirs(root)
        total_pause += len(pause_dirs)

        for pause_dir in pause_dirs:
            rm_csv = pause_dir / RM_FILE
            concat_csv = pause_dir / CONCAT_FILE
