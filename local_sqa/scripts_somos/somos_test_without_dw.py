...
RESULT_ROOT = SCRIPT_DIR / "results" / "SOMOS" / "without_dw"
PACKET_DIR = RESULT_ROOT / "packets"
...
def mean_unweighted(values) -> float:
    if not values:
        return float("nan")
...
def packet_path(system_id: str) -> Path:
    tag = SOMOS_TEST_LIST.parent.name  # "clean" or "full"
    return PACKET_DIR / f"packet_somos_{tag}_{system_id}_t{int(TARGET_SECONDS)}_s{START_INDEX}.csv"
...
def out_dir_for(mode: str, system_id: str) -> Path:
    if mode == "forward":
        return RESULT_ROOT / f"without_{system_id}"
    if mode == "reverse":
        return RESULT_ROOT / f"without_rev_{system_id}"
    if mode == "random":
        return RESULT_ROOT / f"without_rand_{system_id}"
...
def run_one(items, system_id: str, mode: str):
    ...
    for i, (utt, mos_t, p, dur) in enumerate(packet, start=1):
        ...
        pred_rm = mean_unweighted(pred_list)
        tgt_rm = mean_unweighted(tgt_list)
    ...
    # concat path, prefix target = UNWEIGHTED mean
    concat_wavs = []
    concat_secs = 0.0
    global_concat_rows = []
    tgt_prefix = []

    for n, (utt, mos_t, p, dur) in enumerate(packet, start=1):
        wav = load_wav(p)
        ...
        tgt_concat = mean_unweighted(tgt_prefix)
