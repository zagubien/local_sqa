...
def parse_somos_list(list_path: Path):
    ...
    for ln in list_path.read_text().splitlines():
        ...
        if not ln:
            continue
        if ln.lower().startswith("utteranceid"):
            continue
        utt, mos = ln.split(",")
...
def pick_system_sequence(items, target_seconds: float):
    ...
    if not filtered:
        # show a hint in case SYSTEM_ID formatting is wrong
        sample = [extract_system_id(u) for (u, _) in items[:20]]
        raise ValueError(f"no items found for SYSTEM_ID={SYSTEM_ID}. example parsed ids from file: {sample}")
