...
OUTPUT_ROOT = Path("/net/vol/zigor/two_speaker_sweep/")
...
def build_interleaved_list(male_list, female_list):
    """M1, F1, M2, F2, ... up to min(len(male_list), len(female_list))."""
    n_pairs = min(len(male_list), len(female_list))
    interleaved = []
    for i in range(n_pairs):
        interleaved.append(male_list[i])
        interleaved.append(female_list[i])
    return interleaved


def concat_interleaved(interleaved_list, n: int) -> np.ndarray:
    if n <= 0:
        return np.zeros(1, dtype=np.float32)
    return np.concatenate(interleaved_list[:n]).astype(np.float32)
...
def get_csv_path() -> Path:
    exp_dir = get_experiment_output_dir()
    run_name = Path(MODEL_DIR).name
    base_prefix = f"{CSV_PREFIX}_{run_name}"
    csv_path = exp_dir / f"{base_prefix}.csv"
    ...
    idx = 1
    while True:
        candidate = exp_dir / f"{base_prefix}_v{idx}.csv"
...
def run_sweep():
    print(f"Loading male chapter from:   {MALE_CHAPTER_DIR}")
    _, male_audio, male_durations = load_sentences(MALE_CHAPTER_DIR)
    print(f"Male sentences: {len(male_audio)}, total {sum(male_durations):.2f} s")

    print(f"\nLoading female chapter from: {FEMALE_CHAPTER_DIR}")
    _, female_audio, female_durations = load_sentences(FEMALE_CHAPTER_DIR)
    print(f"Female sentences: {len(female_audio)}, total {sum(female_durations):.2f} s")

    interleaved = build_interleaved_list(male_audio, female_audio)
    max_n = len(interleaved)
    total_len_full = sum(len(w) for w in interleaved) / SAMPLING_RATE
    print(f"\nInterleaved segments: {max_n}, full length {total_len_full:.2f} s")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for n in range(1, max_n + 1):
            wav = concat_interleaved(interleaved, n)
